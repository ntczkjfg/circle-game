...
class Player(Entity):
    ...
    def collide(self, entity):
        ...
        elif entity.type == 19: # Bomb, explodes after 5 seconds damaging player and killing entities within radius
            bomb = Entity(c = entity.c, r = entity.r, speed = 0, typ = 20, player = self, parent = self.game)
            bomb.bombTimer = 5*60
            self.game.entities.append(bomb)
        elif entity.type == 20: # Activated bomb entity
            if entity.bombTimer <= 0 and self.iframes == 0:
                self.lifeCount -= 1
                self.iframes = 30
